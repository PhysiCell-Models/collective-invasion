...
def plot_cell_histories_for_movie(starting_index: int, sample_step_interval: int, number_of_samples: int, output_plot: bool,
                     show_plot: bool, naming_index: int, produce_for_panel: bool): #https://www.geeksforgeeks.org/args-kwargs-python/
    """
                ...
                if (child.attrib['id'] in d.keys()):
                    d[child.attrib['id']] = np.vstack(( d[child.attrib['id']],
                                                       [float(circle.attrib['cx']) - x_coordinate_transform,
                                                        float(circle.attrib['cy']) - y_coordinate_transform, float(circle.attrib['r']) ]))
                else:
                    d[child.attrib['id']] = np.array([float(circle.attrib['cx']) - x_coordinate_transform,
                                                      float(circle.attrib['cy']) - y_coordinate_transform, float(circle.attrib['r']) ])
                    d_attributes[child.attrib['id']] = circle.attrib['fill']
                #### Alternative method that would extract the cell radius. Would use if using patches instead of scatter. Woudl allow a direct physical analog to the cell radius.
                # if (child.attrib['id'] in d.keys()):
                #     d[child.attrib['id']] = np.vstack(( d[child.attrib['id']],
                #                                        [float(circle.attrib['cx']) - x_coordinate_transform,
                #                                         float(circle.attrib['cy']) - y_coordinate_transform, float(circle.attrib['r']) ]))
    ...
    for key in d.keys():
        if (len(d[key].shape) == 2):
            x = d[key][:, 0]
            y = d[key][:, 1]
            # r = d[key][:, 2] #would only use if using the circle patches not scatter
...
def plot_cell_tracks_from_svg(starting_index: int, sample_step_interval: int, number_of_samples: int, output_plot: bool,

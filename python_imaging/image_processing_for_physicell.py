...
def generic_plotter (starting_index: int=0, sample_step_interval: int=1, number_of_samples: int=120, file_name: str= None, options=None):
    ...
    cell_positions, cell_attributes, title_str, plot_x_extend, plot_y_extend = load_cell_positions_from_SVG(starting_index, sample_step_interval, number_of_samples)
    ...
    # print(fig)
    plt.show()
    create_figure_from_SVG(cell_positions, cell_attributes, fig)
    fig = plt.gcf()
    ...
    # plot_figure(fig, title_str, plot_x_extend, plot_y_extend, file_name, options)
...
def plot_figure(fig, title_str, plot_x_extend, plot_y_extend, file_name, options: dict=None): ###### This should probably have to have options??????? Why though???
    if options is None:
        options= {"output_plot": True,
                  "show_plot": True,
                  "produce_for_panel": False
...
def load_cell_positions_from_SVG(starting_index: int, sample_step_interval: int, number_of_samples: int):
        """
...
def create_figure_from_SVG (cell_positions: dict, cell_attributes: dict, f=fig):
    d = cell_positions

...
In this framework [[1](#references)] for modeling the ECM and cell-ECM interactions, we divide the ECM into volumetric elements that track local ECM density, alignment, and overall anisotropy (local microstructure). Individual cell agents can locally remodel each of these properties, while these properties can influence cell behavior including changes in migration speed, chemotactic response, ECM contact guidance, proliferation, death, secretion, and differentiation. It is implemented as an extension of the open source package PhysiCell [[2]](#references) and can readily be used to incorporate local ECM effects into agent-based models, in particular through PhysiCell rules [[3]](#references). 
...
There are four example models, several variants of the leader-follower model, and simple test models. See the below `Compling and running sample models` below for details on those models. 
...
#### _Invasive cellular front_:

`make` - compiles the AMIGOS-invasion executable

The following output directories will need made to run the simulations below: `simple_tests_Painter_random`, `simple_tests_Painter_parallel`, `simple_tests_Painter_orthgonal`, and `simple_tests_Painter_mixed`. 

- `./AMIGOS-invasion config/simple_test_Painter_random.xml` - simulation of cells being pushed into pre-structured ECM. In this case, the ECM elements are randomly oriented. Left image of Figure 3 in [[1](#references)]. Inspired by Painter 2009  [[4](#references)] (see Figure 10).
- `./AMIGOS-invasion config/simple_test_Painter_parallel.xml` - simulation of cells being pushed into pre-structured ECM. In this case, the ECM element orientations are parallel to the advancing cell front. Left-center image of Figure 3 in [[1](#references)]. Inspired by Painter 2009 [[4](#references)] (see Figure 10).
- `./AMIGOS-invasion config/simple_test_Painter_orthogonal.xml` - simulation of cells being pushed into pre-structured ECM. In this case, the ECM element orientations are orthogonal to the advancing cell front. Right-center image of Figure 3 in [[1](#references)]. Inspired by Painter 2009 [[4](#references)] (see Figure 10).
- `./AMIGOS-invasion config/simple_test_Painter_mixed.xml` - simulation of cells being pushed into pre-structured ECM. In this case, the ECM element orientations are mixed, with separate regions parallel or orthogonal to the advancing cell front. Right image of Figure 3 in [[1](#references)]. Inspired by  Painter 2009 [[4](#references)] (see Figure 10).
...
- `./fibrosis config/fibrosis.xml` - simulated tissue insult is cleared by macrophages, which recruit fibroblasts that increase ECM density in the presence of macrophages, leading to a relatively impenetrable hyperdense ECM surrounding a region that is much less dense in ECM and more fluid-filled (Figure 4 from [[1](#references)]).
...
- `./invasive_carcinoma config/invasive_carcinoma.xml` - simulation of basement membrane degradation by tumor recruited fibroblasts, leading to invasion of stroma by previously _in situ_ tumor (Figure 5 from [[1](#references)]).
...
- `./AMIGOS-invasion writing_only.xml`- No contact guidance in the follower cell population - notably lacks outward migration of fiber following cells ("followers") (Figure 6a from [[1](#references)])
- `./AMIGOS-invasion reading_only.xml` - No production of directional cues in ECM for followers to follow - notably lacks outward migration of fiber following cells (Figure 6b from [[1](#references)])
- `./AMIGOS-invasion writing_and_reading.xml` - Enables the two cell populations (leader and follower), with leaders producing paths in the ECM that followers follow (stigmergy) (Figure 6c from [[1](#references)])
- `./AMIGOS-invasion leader_follower_instant_speed_080.xml` - Produces stigmery (Figure 7a from [[1](#references)])
- `./AMIGOS-invasion leader_follower_instant_speed_050.xml` - Produces leader-follower collective migration (Figure 7b from [[1](#references)])
- `./AMIGOS-invasion leader_follower_instant_speed_010.xml` - Produces no change in cell arrangement/pattern (Figure 7c from [[1](#references)])
- `./AMIGOS-invasion leader_follower_model.xml` - Demonstrates leader-follower collective migration, even in the case of non-instant ECM remodeling (Figure 8b from [[1](#references)])
...
All the above models (and in general any recently created PhysiCell model) can be run via the PhysiCell Studio ([User Guide](https://github.com/PhysiCell-Tools/Studio-Guide/blob/main/README.md) and reference [[5]](#references)). 
...
Note that the ECM-based models are modifiable in the regular version of the studio, but the  ECM visualization is not. Please see the PhysiCell Studio Guide and article for general information and details on the Studio.

NOTE: On Windows, the path separator is still `\`. This is true for the Studio also!!!
...
[4] Painter, K.J.. Modelling cell migration strategies in the extracellular matrix.  J. Math. Biol. 58, 511–543, 2009.https://doi.org/10.1007/s00285-008-0217-8

[5] Heiland, R., Bergman, D., Lyons, B., Waldow, G., Cass, J., Rocha, H.L. da, Ruscone, M., Noël, V., Macklin, P. PhysiCell Studio: a graphical tool to make agent-based modeling more accessible. Gigabyte 2024, 1–19, 2024. https://doi.org/10.46471/gigabyte.128


[6] Duggan, B.S., Metzcar, J., and Macklin, P. DAPT: A package enabling distributed automated parameter testing. Gigabyte 2021, 1–10, 2021. https://doi.org/10.46471/gigabyte.22
